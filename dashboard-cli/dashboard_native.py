...
def get_systemd_status(service_name):
    """Checks native systemd service state and formats up/down indicators."""
    try:
        result = subprocess.run(
...
def get_pihole_stats():
    """Queries Pi-hole v6 core CLI directly to gather absolute blocked counts."""
    try:
        result = subprocess.run(
...
def check_jeeves_logs():
    """Scans recent J.E.E.V.E.S journalctl output for runtime execution errors."""
    try:
        result = subprocess.run(
...
def main():
    ...
    # Minecraft Natively Polled Service Status
