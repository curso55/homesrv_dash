...
`conf.yml` and `dark.css` here are the actual files the running `dashy`
container mounts (see the service block in `../docker-compose.yml`) — edit
them in place and re-deploy per below.

## What's NOT in this repo

Excluded via `.gitignore` at the repo root:

- **`dashy.env`** — holds `DASHY_PIHOLE_API_KEY` / `DASHY_UPTIME_KUMA_API_KEY`.
  Mode `600`, never committed. (Both keys are currently unused — see "Known
  Dashy bugs" below.)
- **`../data/`** — TLS certs, Uptime Kuma/MySpeed sqlite databases. Runtime
  state, not config.
- **`../config/`** — currently empty; left over from Homepage, root-owned.
