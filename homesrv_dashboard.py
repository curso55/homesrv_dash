...
# Global Cache to prevent hammering APIs on rapid refreshes
FACTORIO_STABLE_CACHE = "Unknown"
LAST_API_CHECK = 0
API_COOLDOWN = 300  # Only check web API every 5 minutes
...
def get_factorio_versions():
    """Fetches local container version and matches it against the cached official API stable build."""
    global FACTORIO_STABLE_CACHE, LAST_API_CHECK
    local_ver = "Unknown"

    try:
        # Verified absolute image path mapping
        res = subprocess.run(
    ...
    # Fetch remote stable version only once every 5 minutes to prevent rendering delay
    now_ts = time.time()
    if FACTORIO_STABLE_CACHE == "Unknown" or (now_ts - LAST_API_CHECK) > API_COOLDOWN:
        try:
            ...
            with urllib.request.urlopen(req, timeout=2) as response:
                ...
                FACTORIO_STABLE_CACHE = data.get("stable", {}).get("headless", "Unknown")
    ...
    if local_ver == "Unknown":
        return ""
    elif FACTORIO_STABLE_CACHE != "Unknown" and local_ver != FACTORIO_STABLE_CACHE:
        return f" {YELLOW}(Update Available: v{FACTORIO_STABLE_CACHE} | Current: v{local_ver}){RESET}"
    else:
...
# --- RUN INTERACTIVE REFRESH LOOP ---
def main():
    try:
        while True:
            run_dashboard()
            time.sleep(2)  # Safely redraws the screen console every 2 seconds
    except KeyboardInterrupt:
        print(f"\n{GREEN}Exiting Central Command Dashboard cleanly.{RESET}\n")
